...
// --- Mejora 1: contador visual de usuarios (tarjetas de estadísticas) ---
async function renderEstadisticas() {
    ...
    grid.innerHTML = `
        <div class="stat-card">
            <span class="stat-numero">${stats.totalUsuarios}</span>
            <span class="stat-label">Usuarios registrados</span>
        </div>
        <div class="stat-card">
            <span class="stat-numero">${stats.usuariosActivos}</span>
            <span class="stat-label">Usuarios activos</span>
        </div>
        <div class="stat-card stat-alerta">
            <span class="stat-numero">${stats.usuariosBloqueados}</span>
            <span class="stat-label">Usuarios bloqueados</span>
        </div>
        <div class="stat-card">
            <span class="stat-numero">${stats.totalJuegos}</span>
            <span class="stat-label">Juegos en catálogo</span>
        </div>
        <div class="stat-card">
            <span class="stat-numero">${stats.mensajesNuevos}</span>
            <span class="stat-label">Mensajes nuevos</span>
        </div>
    `;
}

// --- Mejora 2: gestión de usuarios (bloquear / desbloquear / eliminar) ---
async function renderTablaUsuarios() {
    ...
    tbody.innerHTML = usuarios.map(u => `
        <tr>
            ...
            <td>
                ${u.rol === 'admin'
                    ? '<span class="estado-pill estado-activo">Administrador</span>'
                    : u.bloqueado
                        ? '<span class="estado-pill estado-bloqueado">Bloqueado</span>'
                        : '<span class="estado-pill estado-activo">Activo</span>'
                }
            </td>
            <td>
                ${u.rol === 'admin' ? '<span style="color:#999;font-size:0.8rem;">Sin acciones</span>' : `
                    ${u.bloqueado
                        ? `<button class="btn-small btn-unblock" data-desbloquear="${u.username}">Desbloquear</button>`
                        : `<button class="btn-small btn-block" data-bloquear="${u.username}">Bloquear</button>`
                    }
                    <button class="btn-small btn-danger-outline" data-eliminar-usuario="${u.username}">Eliminar</button>
                `}
            </td>
        </tr>
    `).join('') || `<tr><td colspan="5">No hay usuarios registrados.</td></tr>`;

    tbody.querySelectorAll('[data-bloquear]').forEach(btn => {
        btn.addEventListener('click', () => bloquearUsuario(btn.dataset.bloquear));
    });
    tbody.querySelectorAll('[data-desbloquear]').forEach(btn => {
        btn.addEventListener('click', () => desbloquearUsuario(btn.dataset.desbloquear));
    });
    tbody.querySelectorAll('[data-eliminar-usuario]').forEach(btn => {
        btn.addEventListener('click', () => eliminarUsuario(btn.dataset.eliminarUsuario));
    });
}

async function bloquearUsuario(username) {
    if (!confirm(`¿Bloquear a "${username}" hasta nuevo aviso? No podrá iniciar sesión ni seguir navegando si está conectado.`)) return;
    const res = await fetch(`/api/admin/usuarios/${username}/bloquear`, { method: 'PATCH', credentials: 'include' });
    if (!res.ok) {
...
async function desbloquearUsuario(username) {
    const res = await fetch(`/api/admin/usuarios/${username}/desbloquear`, { method: 'PATCH', credentials: 'include' });
    if (!res.ok) return;
    renderTablaUsuarios();
    renderEstadisticas();
}
...
async function eliminarUsuario(username) {
    ...
    const res = await fetch(`/api/admin/usuarios/${username}`, { method: 'DELETE', credentials: 'include' });
    if (!res.ok) {
        const data = await res.json();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (sesion.bloqueado) {
        window.location.href = '/welcome/welcome.html?bloqueado=1';
        return;
    }
